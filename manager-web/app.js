...
function clearData(){activeRequestId='';for(const id of ['note','due','taskNote','hotel','price','operator','meal','carrier'])$(id).value='';for(const id of ['analytics','tasks','assignment','summary','quotes','activities'])$(id).replaceChildren();$('detail').hidden=true;}
...
async function api(path,body){
 ...
 if(response.status===409){const conflict=await response.json();throw Error('Заявка уже назначена: '+(conflict.assignment?.name||'другому менеджеру')+'.');}
...
function renderAssignment(assignment){$('assignment').replaceChildren();if(assignment?.assigned){paragraph($('assignment'),'Ответственный: '+assignment.name);return;}paragraph($('assignment'),'Ответственный ещё не назначен.');const button=document.createElement('button');button.textContent='Взять в работу';button.addEventListener('click',()=>claimRequest(button));$('assignment').append(button);}
...
async function openRequest(id){
 ...
 renderAssignment(t.assignment);paragraph($('summary'),[r.primary_destination,r.departure_city,r.dates_text,r.adults?`${r.adults} взрослых`:'',(r.children||[]).length?'Дети: '+r.children.map(c=>c.age).join(', '):'',r.budget_amount?`Бюджет: ${r.budget_amount} ${r.budget_currency||''}`:''].filter(Boolean).join(' · '));
