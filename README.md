...
<img src="docs/dialog.svg" alt="Диалог бота: город вылета, подтверждение дат, состав с детьми, заявка и живые цены Tutu" width="720">
...
**Работает на polling — и это не прихоть.** Хостинг фильтрует адреса Telegram
в обе стороны. Симптом узнаваемый и обманчивый: `getWebhookInfo` показывает
`Connection timed out` с растущей очередью, а в логах nginx **ни одного**
запроса от Telegram. Бот при этом здоров, `active (running)`, `/health` — 200.
Ни сертификат, ни секрет, ни конфиг не помогут: маршрутом владеет провайдер.

`BOT_MODE=polling` разворачивает направление — бот сам ходит за апдейтами, и
нужен только исходящий канал. Вебхук остаётся режимом по умолчанию, оба
транспорта идут через один `dispatch_update()`, так что разъехаться не могут.

Исходящий канал тоже оказался с сюрпризом: DNS отдавал `149.154.166.110` —
таймаут, при этом `149.154.167.220` из того же пула отвечал. Полезная
формулировка для тикета провайдеру — «этот адрес закрыт, тот открыт», а не
«телеграм не работает». Закрепление рабочего адреса в `/etc/hosts` помогает,
но это подпорка: Telegram меняет адреса, и она протухнет.
