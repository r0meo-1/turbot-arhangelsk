...
| **Проверить** | `pytest -q` · [CI](https://github.com/r0meo-1/turbot-arhangelsk/actions) · живые ссылки ниже |
...
| | |
|--|--|
| **Health** | [turbot-arhangelsk.onrender.com/health](https://turbot-arhangelsk.onrender.com/health) — JSON со статусом, флагами и счётчиком заявок |
| **Политика ПДн** | [/privacy](https://turbot-arhangelsk.onrender.com/privacy) — бот отдаёт её сам, без отдельного хостинга |

> ⏱ **Первый запрос просыпает инстанс — это до минуты.** Free-tier Render
> засыпает после 15 минут простоя. Не баг: осознанный размен, 750 бесплатных
> часов в месяц лучше потратить на рабочее время, чем на круглосуточный пинг
> (см. [`keep-alive.yml`](.github/workflows/keep-alive.yml)). Второй запрос — 8 секунд.

**Демо-режим.** Публичный инстанс не собирает настоящие ПДн: он честно
предупреждает, что это витрина, заявка не уходит в турагентство, а телефон
хранится замаскированным (`+7916***4567`). Цены на перелёт при этом
настоящие — они приходят из Tutu.ru по MCP. На своей VM режим выключен и бот
работает как обычно.
